function [LL,b_eta,th_est,res,LR,u] = test_Johansen_VECM_MFI1(tily,k,alpha,fr,S,Joh_j,plots);
% Calculates the Johansen test using the VECM representation 
%
% SYNTAX: [LL,b_eta,alphahat,betahat,Phi] = test_Johansen_VECM(y,k,alpha,freq,S,Joh_j,plots);
%
% INPUT: y  ... Txs observations
%        k  ... integer; lag length (if negative estimated).
%        alpha ... real; significance level.
%        freq  ... d vector of reals in [0,1]; frequencies to include in
%                  the test.
...
% provide unit root structure urs. 
urs(:,1)=fr(:); 
urs(:,2)=1;
for j=1:length(fr) 
...
[~,~,~,LR] = VECM_MFI1(tily,k,S,urs,Joh_j);
...
if isempty(urs)
    disp('Specification tests suggest no unit root after all!');
    th_est = VAR(tily,zeros(T,0),k,'LS');
    res = [];
else
    [LL,th_est,res,LR,~,u] = VECM_MFI1(tily,k,S,urs,Joh_j);
end
