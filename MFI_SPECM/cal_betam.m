function [beta,LL] = cal_betam(S00,S01,S11,r);
...
if (r<s) &&(s>1) && (r>0)
    ...
    bb1n = [real(ibb(r+1:end,:)),imag(ibb(r+1:end,:))];

    % numerical optimization
    try
        [betamn,LL] = fminunc(@(x) ll_det(x,tS11,tS11d0,r),bb1n(:));
    catch
        [betamn,LL] = fminunc(@(x) ll_det(x,tS11,tS11d0,r),bb1n(:)+randn(length(bb1n(:)),1)*0.1);
    end

    % convert back. 
    beta = TT*cal_beta_par(sn,r,betamn);
    LL = (LL + log(det(S00)));
else
    if (r==s)
        beta = eye(2*r);
...
%%
function dd = ll_det(x,S11,S11d0,r);
% logdet det(b'S11d0 b)/det(b'S11 b).

s = size(S11,1)/2;
beta = cal_beta_par(s,r,x);
dd = log(det(beta'*S11d0*beta)/det(beta'*S11*beta));

%%
function Betam = cal_beta_par(s,r,x);
%
bb1 = reshape(x,s-r,2*r);
bb0 = [eye(r);bb1(:,1:r)+sqrt(-1)*bb1(:,r+1:end)];
