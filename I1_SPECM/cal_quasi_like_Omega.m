function [qlike,tres] = cal_quasi_like_Omega(param,th,y,s,m,n,c,Pbullet,restrict);
% calculates the quasi likelihood for theta structure theta for parameter
% vector param including parameters for innovation variance.
%
% SYNTAX: [qlike,tres] = cal_quasi_like(param,y,s,m,n,c,Pbullet,restrict);
%
% INPUTS:  param ... d x 1 vector of parameter values for Omega (fed into fill_lowtri;
%                       see there for description)
%          th    ... theta structure of system.
...
Omega = fill_lowtri(param,s);

% extract matrices from th
A = th.A;
K = th.K;
C = th.C;
D = th.D;
...
if Pbullet>0
    ...
    Omegat =  tres(1:(Ts-1),:)'*tres(1:(Ts-1),:)/(Ts);
